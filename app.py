...
def retry_api_call(func, max_retries=3, backoff_factor=1):
...
def get_credentials():
    ...
    try:
        ...
        logger.error(f"Error loading credentials from session: {e}")
        return None
...
def inbox():
    ...
    try:
        ...
        def get_messages():
            return service.users().messages().list(userId='me', maxResults=50).execute()
        
        results = retry_api_call(get_messages)
        ...
        try:
            profile = retry_api_call(get_user_profile)
        ...
        for msg in messages:
            try:
                ...
                msg_data = retry_api_call(get_message)
                ...
                try:
                    ...
                    logger.error(f"Error classifying email {msg['id']}: {e}")
                    predictions = {}
                    top_tags = []
        ...
        if e.resp.status == 401:
            session.clear()
            return redirect(url_for('login'))
        return f"Gmail API Error: {e}. Please try again later."
    except Exception as e:
...
def download_attachment(email_id, attachment_id):
    ...
    try:
        ...
        # Get attachment with retry logic
        def get_attachment():
        ...
        attachment = retry_api_call(get_attachment)
